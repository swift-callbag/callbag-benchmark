public let mergeHotObservableBenchmarks = BenchmarkSuite(
  name: "Merge"
) { suite in
  let count = samples
  ...
  suite.benchmark("CallbagKit", settings: Iterations(count)) {
    ...
    _ = pipe(
      ...
      map(Double.init),
      filter { $0.truncatingRemainder(dividingBy: 2) == 0 },
      observe({ _ in })
    )
  ...
  suite.benchmark("ReactiveSwift", settings: Iterations(count)) {
    ...
    ReactiveSwift.Signal
      .merge(subscriber1, subscriber2, subscriber3, subscriber4)
      .map(Double.init)
      .filter { $0.truncatingRemainder(dividingBy: 2) == 0 }
      .observe { _ in }
...
public let mergeColdObservableBenchmarks = BenchmarkSuite(
  name: "Merge"
) { suite in
  let count = samples
  let range = (1...((iterations / 2) + 1))
