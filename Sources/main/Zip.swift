public let zipHotObservableBenchmarks = BenchmarkSuite(
  name: "Zip"
) { suite in
  let count = samples
  ...
  suite.benchmark("ReactiveSwift", settings: Iterations(count)) {
    ...
    ReactiveSwift.Signal
      .zip(subscriber1, subscriber2, subscriber3, subscriber4)
      .map { $0 + $1 + $2 + $3 }
      .filter { $0 % 2 == 0 }
      .observe { _ in }
  ...
  suite.benchmark("RxSwift", settings: Iterations(count)) {
    ...
    RxSwift.Observable
      .zip(source1, source2, source3, source4)
      .map { $0 + $1 + $2 + $3 }
      .filter { $0 % 2 == 0 }
      .subscribe()
...
public let zipColdObservableBenchmarks = BenchmarkSuite(
  name: "Zip"
) { suite in
  let count = samples
  let range = (1...((iterations / 2) + 1))
